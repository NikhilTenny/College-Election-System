<?php 
	...
	if(($_GET['male']) && ($_GET['female'])) {
		...
		$m_uid = $_GET['male'];
		$f_uid = $_GET['female'];
		$mupQ = "update candidate set Votes = Votes + 1 where User_id = $m_uid";
		$fupQ = "update candidate set Votes = Votes + 1 where User_id = $f_uid";
		if(mysql_query($mupQ,$con))
			if(mysql_query($fupQ,$con)) {
				$upvotersQ = "update voters_list set Vote_Status =  1 where User_id = $id;";
